...
## v1.3.0 签名优化（参考 FluidVoice 最佳实践）

### 改进 1：xcodebuild 禁用签名，脚本统一管理

参考 FluidVoice 项目的做法，将 Xcode build 阶段的签名完全禁用（`CODE_SIGN_IDENTITY="-"`, `CODE_SIGNING_REQUIRED=NO`, `CODE_SIGNING_ALLOWED=NO`），由 `build-dmg.sh --sign` 统一控制签名。

**好处**：
- 避免 Xcode 项目设置与脚本签名冲突
- 签名身份和 entitlements 完全由脚本控制，保证一致性
- 不受开发者本地 Xcode Signing 配置影响

### 改进 2：移除 `--deep`，改用分层签名

Apple 不推荐使用 `codesign --deep`，因为它无法正确传递不同组件的 entitlements。改为显式分层签名：
- **内层组件**（Frameworks、辅助工具）：使用 `INNER_SIGN_ARGS`（无 entitlements）
- **主 bundle**：使用 `BUNDLE_SIGN_ARGS`（带 entitlements）

### 改进 3：签名验证加入 `--strict`

验证时使用 `codesign --verify --strict --verbose=2`，比之前更严格地检查签名有效性。

### 签名信息

| 属性 | 值 |
|------|-----|
| Authority | `5JNN9A6Z6X` |
| TeamIdentifier | `8RQVLSP2SC` |
| Hardened Runtime | Yes (`flags=0x10000(runtime)`) |
| Entitlements | `TransFlow.entitlements`（麦克风权限等） |
| DMG 签名 | Yes（同一证书） |
